...
async function main() {
  ...
  const whale1 = await ethers.getImpersonatedSigner(maycTraders[0]);
  const whale1Info = await chViewer.getTraderPositionInfo(maycAmmAddress, whale1.address);
  console.log(`${whale1.address}'s collateral`, formatEther(whale1Info.margin));

  const whale2 = await ethers.getImpersonatedSigner(maycTraders[1]);
  const whale2Info = await chViewer.getTraderPositionInfo(maycAmmAddress, whale2.address);
  console.log(`${whale2.address}'s collateral`, formatEther(whale2Info.margin));

  const whale3 = await ethers.getImpersonatedSigner(maycTraders[2]);
  const whale3Info = await chViewer.getTraderPositionInfo(maycAmmAddress, whale3.address);
  console.log(`${whale3.address}'s collateral`, formatEther(whale3Info.margin));
  ...
  console.log("mayc vault before ", formatEther(maycVault));
  console.log("mayc IF budget before", formatEther(IFBudget));
  console.log("staking reward before", formatEther(stakingReward));

  // whale1 closes
  const closer = whale3;
  console.log(`${closer.address} closes his position`);
  // // partial close for big position
  // await ch
  //   .connect(closer)
  //   .openPosition(
  //     maycAmmAddress,
  //     whale3Info.positionSize.isNegative() ? 0 : 1,
  //     whale3Info.positionSize.div(BigNumber.from(2)).abs(),
  //     1,
  //     0,
  //     false
  //   );
  await ch.connect(closer).closePosition(maycAmmAddress, 0);

  const promises = maycTraders.map((trader) => chViewer.getTraderPositionInfo(maycAmmAddress, trader));
  const resArray = await Promise.all(promises);
  const posInfos = resArray.map((info, index) => ({ ...info, trader: maycTraders[index] }));
  const underwaterTraders = posInfos.filter((info) => !info.positionSize.isZero() && info.isLiquidatable).map((info) => info.trader);
  console.log("underwater Traders", underwaterTraders);
  ...
  console.log("mayc vault after ", formatEther(maycVault2));
  console.log("mayc IF budget after", formatEther(IFBudget2));
