import { PositionChanged, PositionLiquidated, MarginChanged, Repeg, UpdateK } from "../generated/ClearingHouse/ClearingHouse";
import { PositionChangedEvent, PositionLiquidatedEvent, MarginChangedEvent, RepegEvent, UpdateKEvent } from "../generated/schema";
import { getPosition, getAmmPosition, calcNewAmmOpenNotional, decimal, getAmm, BI_ZERO, getTraderDayData, getDayTradeData } from "./helper";
...
export function handlePositionChanged(event: PositionChanged): void {
  ...
  //
  let traderDayData = getTraderDayData(event, event.params.trader);
  traderDayData.tradingVolume = traderDayData.tradingVolume.plus(event.params.positionNotional);
  traderDayData.fee = traderDayData.fee.plus(event.params.fee);
  traderDayData.fundingPayment = traderDayData.fundingPayment.plus(event.params.fundingPayment);
  traderDayData.realizedPnl = traderDayData.realizedPnl.plus(event.params.realizedPnl);

  //
  // update day trade data
  //
  let dayTradeData = getDayTradeData(event, event.params.amm);
  dayTradeData.volume = dayTradeData.volume.plus(event.params.positionNotional);
  dayTradeData.txCount = dayTradeData.txCount.plus(BigInt.fromI32(1));
...
export function handleRepeg(event: Repeg): void {
  let entity = new RepegEvent(event.transaction.hash.toHexString() + "-" + event.logIndex.toString());
  entity.amm = event.params.amm;
...
export function handleUpdateK(event: UpdateK): void {
  let entity = new UpdateKEvent(event.transaction.hash.toHexString() + "-" + event.logIndex.toString());
  entity.amm = event.params.amm;
  entity.baseAssetReserve = event.params.baseAssetReserve;
  entity.quoteAssetReserve = event.params.quoteAssetReserve;
  entity.cost = event.params.cost;
